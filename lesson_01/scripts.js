async function getUsers (){
    const response = await fetch("http://10.11.0.193:3000/");
    ...
    if (response.ok){
        response.json().then(data => {
            data.forEach(user => {
                ...
                actionElement.innerHTML = status === 1 ? `<div class="blockButton"><p>Блокировать</p></div>` : `<div class="unblockButton"><p>Восстановить</p></div>`;
            });
        });
    }
}
